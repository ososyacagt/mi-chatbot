...
export async function applyBusinessRules(tenantId, cartItems) {
  ...
  console.log('[rules] Reglas activas:', rules.length)
  ...
  for (const rule of rules) {
    ...
    try {
      ...
      if (tipo === RULE_TYPES.GIFT_PURCHASE) {
        console.log('[gift] Evaluando regla gift_purchase')
        console.log('[gift] Items en carrito:', processedItems.length)
        ...
        console.log('[gift] Min subtotal requerido:', condiciones.min_subtotal)
        console.log('[gift] Condicion cumplida:', subtotal >= condiciones.min_subtotal)
        console.log('[gift] gift_product_id:', acciones.gift_product_id)

        const alreadyGifted = giftItems.some(
          g => g.productId === acciones.gift_product_id
        )
        console.log('[gift] Ya existe regalo:', alreadyGifted)
