...
async function sendWithClaude(model, systemPrompt, messages) {
...
async function sendWithOpenAI(model, systemPrompt, messages) {
...
export async function sendMessage({ provider, model, systemPrompt, messages }) {
  console.log(`[AI Provider] Using ${provider}:${model}`);

  try {
    switch (provider) {
      case "claude":
        return await sendWithClaude(model, systemPrompt, messages);
      case "openai":
        return await sendWithOpenAI(model, systemPrompt, messages);
      case "groq":
