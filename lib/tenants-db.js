...
function mapTenantFromDb(dbRecord) {
  ...
  return {
    ...
    ecommerceModes: dbRecord.ecommerce_modes || [],
    ecommerceMode: (() => {
      // Prefiere ecommerce_modes[] (multi-modo), con fallback al campo legacy ecommerce_mode
      const modes = dbRecord.ecommerce_modes || [];
      if (modes.length > 0) return modes[0];
      return dbRecord.ecommerce_mode || 'none';
    })(),
