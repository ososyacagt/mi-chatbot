...
export async function checkStock(productId, cantidad, variantId) {
  ...
  if (variantId) {
    ...
    console.log('[checkStock] Variant:', { variantId, stock, stockType: typeof data.stock, qty, qtyType: typeof cantidad, result: stock >= qty });
    return stock >= qty;
  } else {
    const { data, error } = await supabase
      .from("products")
    ...
    const stock = parseInt(data.stock, 10);
    const qty = parseInt(cantidad, 10);
    console.log('[checkStock] Product:', { productId, stock, stockType: typeof data.stock, qty, qtyType: typeof cantidad, esServicio: data.es_servicio, result: stock >= qty });
    return stock >= qty;
  }
}
